...
function parseSales(wb, sheetNames){
  const out = []; const warn = []; let skipped = 0;
  for (const sn of sheetNames){
    const seen = new Map();   // นับต่อชีท: ชีทที่เนื้อหาเหมือนกันจะได้ id ชุดเดียวกันแล้วถูกกันซ้ำ
...
/* ---- ลายนิ้วมือของชีท: จำนวนแถว + วันแรก + วันสุดท้าย ----
   อ่านแค่ 2 เซลล์ต่อชีท จึงเร็วแม้ไฟล์มีหลายแสนแถว */
function sheetFingerprint(wb, name){
  const ws = wb.Sheets[name];
  if (!ws || !ws['!ref']) return null;
  const R = XLSX.utils.decode_range(ws['!ref']);
  let hr = -1, dc = -1;
  for (let r = R.s.r; r <= Math.min(R.s.r + 11, R.e.r) && hr < 0; r++){
    for (let c = R.s.c; c <= R.e.c; c++){
      const cell = ws[XLSX.utils.encode_cell({ r, c })];
      const v = cell ? clean(cell.v) : '';
      if (/^(เวลาสั่งซื้อ|วันที่สั่งซื้อ|วันที่|วันที่ชำระ|วันที่ชำระเงิน)$/.test(v)){ hr = r; dc = c; break; }
    }
  }
  if (hr < 0) return null;
  const at = r => { const cell = ws[XLSX.utils.encode_cell({ r, c: dc })]; return cell ? toISO(cell.v) : null; };
  // บางชีทมีขอบเขตค้างเป็นแถวว่างหลายพันแถว จึงค้นหาแถวสุดท้ายที่มีข้อมูลจริงแบบแบ่งครึ่ง
  const has = r => { const c = ws[XLSX.utils.encode_cell({ r, c: dc })]; return !!(c && c.v !== null && c.v !== ''); };
  let lo = hr + 1, hi = R.e.r;
  while (lo < hi){
    const mid = Math.ceil((lo + hi) / 2);
    if (has(mid)) lo = mid; else hi = mid - 1;
  }
  const lastRow = lo;
  // สุ่มอ่านเลขออเดอร์ 4 จุด เผื่อสองชีทบังเอิญมีจำนวนแถวและช่วงวันเท่ากันแต่เนื้อหาต่างกัน
  const oc = dc > R.s.c ? dc - 1 : dc + 1;
  const sample = [0.13, 0.37, 0.61, 0.89].map(f => {
    const r = hr + 1 + Math.floor((lastRow - hr - 1) * f);
    const c = ws[XLSX.utils.encode_cell({ r, c: oc })];
    return c ? String(c.v) : '';
  }).join(',');
  return { rows: lastRow - hr, first: at(hr + 1), last: at(lastRow), sample };
}
function coverLabel(fp){
  if (!fp || !fp.first) return '';
  const a = `${TH_MON_S[mOf(fp.first)]} ${String(beYear(yOf(fp.first))).slice(2)}`;
  if (!fp.last || fp.first.slice(0, 7) === fp.last.slice(0, 7)) return a;
  return `${a} – ${TH_MON_S[mOf(fp.last)]} ${String(beYear(yOf(fp.last))).slice(2)}`;
}
...
function defaultSheets(wb, kind){
  ...
  if (kind !== 'expense'){
    const seen = new Map();
    return names.map(n => {
      const fp = sheetFingerprint(wb, n);
      const key = fp ? `${fp.rows}|${fp.first}|${fp.last}|${fp.sample}` : 'x' + n;
      const dupOf = seen.get(key);
      if (!dupOf) seen.set(key, n);
      return { name:n, on: !dupOf,
               note: dupOf ? `เนื้อหาซ้ำกับชีท "${dupOf}"` : coverLabel(fp) };
    });
  }
